...
作者是 [Datawhale](https://github.com/datawhalechina)，一个专注于数据科学与 AI 领域的开源组织(有 100+ 个仓库，超级厉害！)
...
不过因为我的 Pandas 版本比较高，
导致部分函数失效，
比如 `DataFrame` 的 `mad` 函数，
在 `pandas` 的 `2.0` 版本之后就废弃了

或者说使用 `any(1)` 或者 `all(1)` 时，
新版的 Pandas 需要显示地指定形参名，
所以建议改为 `any(axis=1)` 和 `all(axis=1)`
